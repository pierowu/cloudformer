...
class UNet2(nn.Module):
    def __init__(self, block=ConvBlock2, dim=32):
        super(UNet2, self).__init__()

        self.dim = dim
        self.ConvBlock1 = block(3, dim, strides=1)
        self.pool1 = nn.Conv2d(dim, dim*2, kernel_size=4, stride=2, padding=1)

        self.ConvBlock2 = block(dim*2, dim*2 , strides=1)
        self.pool2 = nn.Conv2d(dim*2 , dim * 4, kernel_size=4, stride=2, padding=1)

        self.ConvBlock3 = block(dim * 4, dim * 4, strides=1)
        self.pool3 = nn.Conv2d(dim * 4, dim * 8, kernel_size=4, stride=2, padding=1)

        self.ConvBlock4 = block(dim * 8, dim * 8, strides=1)
        self.pool4 = nn.Conv2d(dim * 8, dim * 16, kernel_size=4, stride=2, padding=1)

        self.ConvBlock5 = block(dim * 16, dim * 16, strides=1)

        self.upv6 = nn.ConvTranspose2d(dim * 16, dim * 8, 2, stride=2)
        self.ConvBlock6 = block(dim * 16, dim * 16, strides=1)

        self.upv7 = nn.ConvTranspose2d(dim * 16, dim * 4, 2, stride=2)
        self.ConvBlock7 = block(dim * 8, dim * 8, strides=1)

        self.upv8 = nn.ConvTranspose2d(dim * 8, dim * 2, 2, stride=2)
        self.ConvBlock8 = block(dim * 4, dim * 4, strides=1)

        self.upv9 = nn.ConvTranspose2d(dim * 4, dim, 2, stride=2)
        self.ConvBlock9 = block(dim * 2, dim* 2, strides=1)

        self.conv10 = nn.Conv2d(dim* 2, 3, kernel_size=3, stride=1, padding=1)
...
# class twin_Attention(nn.Module):
...
class WindowAttention_lepe(nn.Module):
    ...
    def get_rpe(self, x, func):
        ...
        x = x.reshape(-1, self.num_heads, C // self.num_heads, H*W).permute(0, 1, 3, 2).contiguous()
        return x, rpe
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # attn_map = attn
        v=v.permute(0,1,3,2).contiguous().reshape(-1,C,H,W)
        ...
        x = (attn @ v+rpe).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x)
        x = self.se_layer(x)
        x = self.proj_drop(x)
        return x
...
class twin_Attention(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
                 proj_drop=0., se_layer=False,sr_ratio=1):
...
class WindowAttention_v2_irpe_orig(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
                 proj_drop=0., se_layer=False):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        if self.rpe_k is not None:
            attn += (self.rpe_k(F.normalize(q, dim=-1)) / torch.norm(k, p=2, dim=-1).unsqueeze(2))

        # image relative position on queries
        if self.rpe_q is not None:
            attn += self.rpe_q(F.normalize(k, dim=-1)).transpose(2, 3)
...
class WindowAttention_v2_irpe(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
                 proj_drop=0., se_layer=False):
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(head_dim,(2 * win_size[0] - 1) * (2 * win_size[1] - 1)))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        relative_position_index = relative_coords.sum(-1)  # Wh*Ww, Wh*Ww
        self.register_buffer("relative_position_index", relative_position_index)
        self.register_buffer("relative_position_index1", torch.arange(self.win_size[0]*self.win_size[1]).unsqueeze(1))
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # attn = (q @ k.transpose(-2, -1))
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1))
        relative_position_contextual_table=torch.matmul(F.normalize(q, dim=-1),self.relative_position_bias_table)
        relative_position_bias=relative_position_contextual_table[:,:,self.relative_position_index1,self.relative_position_index]
        # relative_position_bias = relative_position_contextual_table[:,:,:,self.relative_position_index.view(-1)].view(
        #     self.win_size[0] * self.win_size[1], self.win_size[0] * self.win_size[1], -1)  # Wh*Ww,Wh*Ww,nH
        # relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        # ratio = attn.size(-1) // relative_position_bias.size(-1)
        # relative_position_bias = repeat(relative_position_bias, 'nH l c -> nH l (c d)', d=ratio)
        k_norm=torch.norm(k,p=2,dim=-1).unsqueeze(2)
        relative_position_bias=relative_position_bias/k_norm
...
class WindowAttention_v2_PA(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0.,
                 proj_drop=0., se_layer=False):
...
class LePEAttention_rpe(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, qkv_bias=False, qk_scale=None,
                 attn_drop=0., proj_drop=0.):
        """Not supported now, since we have cls_tokens now.....
        """
        ...
        if idx == -1:
            ...
            H_sp, W_sp = self.resolution, self.split_size
            self.win_size=(self.resolution[0], self.split_size)
        elif idx == 1:
            W_sp, H_sp = self.resolution, self.split_size
            self.win_size = (self.split_size,self.resolution[0])
        else:
            print("ERROR MODE", idx)
            exit(0)
        self.H_sp = H_sp
...
class csWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        self.attns = nn.ModuleList([
            LePEAttention_rpe(
                dim // 2, resolution=self.input_resolution, idx=i,
...
class twin_LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False,sr_ratio=4):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        ...
        self.norm1 = norm_layer(dim)
        self.attn = twin_Attention(
            dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, se_layer=se_layer,sr_ratio=sr_ratio)
    ...
    def forward(self, x, mask=None):
        ...
        # W-MSA/SW-MSA
        x = self.attn(x, mask=attn_mask)  # nW*B, win_size*win_size, C
...
class LeWinConvBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
    ...
    def forward(self, x, mask=None):
        ...
        x=self.conv2(self.attn(self.conv1(x)))
        x=x.permute(0,2,3,1)
...
class LeWinTransformerBlock_v2(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
        ...
        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention_v2_irpe(
            dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, se_layer=se_layer)
...
class LeWinTransformerBlock_PA(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
        ...
        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention_v2_PA(
            dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, se_layer=se_layer)
    ...
    def forward(self, x, mask=None):
        ...
        x_windows = window_partition(shifted_x, self.win_size)  # nW*B, win_size, win_size, C  N*C->C
        #PA
        x_windows=self.pos(x_windows.permute(0,3,1,2)).permute(0,2,3,1).contiguous()
        x_windows = x_windows.view(-1, self.win_size * self.win_size, C)  # nW*B, win_size*win_size, C
...
#########################################
########### LeWinTransformer #############
class LeWinTransformerBlock_sandwich(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
        ...
        self.norm1 = norm_layer(dim)
        self.norm_sandwich = norm_layer(dim)
        self.attn = WindowAttention(
            dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, se_layer=se_layer)
    ...
    def forward(self, x, mask=None):
        ...
        x = shortcut + self.drop_path(self.norm_sandwich(x))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        del attn_mask
        return x
...
class LeWinTransformerBlock_v2_irpe(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False):
        ...
        self.norm1 = norm_layer(dim)
        self.attn = WindowAttention_v2_irpe_orig(
            dim, win_size=to_2tuple(self.win_size), num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            token_projection=token_projection, se_layer=se_layer)
...
class BasiccswinLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False,split_size=2):
        ...
        self.blocks = nn.ModuleList([
            csWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                ...
                                # shift_size=0,
                                 ...
                                 drop=drop, attn_drop=attn_drop,
                                 drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                 norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer,split_size=split_size)
            for i in range(depth)])
...
class BasicUniformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False):
        ...
        # build blocks
        self.blocks = nn.ModuleList([
            LeWinConvBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, win_size=win_size,
                                 # shift_size=0 if (i % 2 == 0) else win_size // 2,
                                shift_size=0,
                                 mlp_ratio=mlp_ratio,
...
class BasictwinLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False):
        ...
        self.use_checkpoint = use_checkpoint
        # build blocks
        self.blocks = nn.ModuleList([LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                                              num_heads=num_heads, win_size=win_size,
                                                              # shift_size=0 if (i % 2 == 0) else win_size // 2,
                                                              shift_size=0,
                                                              mlp_ratio=mlp_ratio,
                                                              qkv_bias=qkv_bias, qk_scale=qk_scale,
                                                              drop=drop, attn_drop=attn_drop,
                                                              drop_path=drop_path[0] if isinstance(drop_path,
                                                                                                   list) else drop_path,
                                                              norm_layer=norm_layer, token_projection=token_projection,
                                                              token_mlp=token_mlp, se_layer=se_layer),
            twin_LeWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads, win_size=win_size,
                                 # shift_size=0 if (i % 2 == 0) else win_size // 2,
                                shift_size=0,
                                 mlp_ratio=mlp_ratio,
...
class CBlock(nn.Module):
    ...
    def flops(self,H,W):
        ...
        flops += H * W* self.dim*self.dim*(1+1)+H * W* self.dim*(3*3+5*5)+self.mlp.flops(H,W)

        print("CBlock:{%.2f}" % (flops / 1e9))
        return flops

class CBlock_insnorm(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
    ...
    def flops(self, H, W):
        ...
        flops += H * W * self.dim * self.dim * (1 + 1) + H * W * self.dim * (3 * 3 + 5 * 5)
        print("CBlock_insnorm_attn:{%.2f}" % (flops / 1e9))
        flops += self.mlp.flops(H, W)
        # print("CBlock_insnorm:{%.2f}" % (flops / 1e9))
        return flops


class UniformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False):
        ...
        # build blocks
        # self.blocks = nn.ModuleList([
        #     LeWinTransformerBlock_v2(dim=dim, input_resolution=input_resolution,
        #                          num_heads=num_heads, win_size=win_size,
        #                          # shift_size=0 if (i % 2 == 0) else win_size // 2,
        #                         shift_size=0,
        #                          mlp_ratio=mlp_ratio,
...
class BasicUniformer_cswin(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False,split_size=1):
...
class Uformer_cswin(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, token_projection='linear', token_mlp='ffn', se_layer=False,
                 dowsample=Downsample, upsample=Upsample, split_sizes=[1, 2, 2, 2, 4, 2, 2, 2, 1],**kwargs):
        super().__init__()
        ...
        # Encoder
        self.encoderlayer_0 = BasicUniformerLayer(dim=embed_dim,
                                                output_dim=embed_dim,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[0],
                                                ...
                                                se_layer=se_layer)
        self.dowsample_0 = dowsample(embed_dim, embed_dim * 2)
        self.encoderlayer_1 = BasicUniformer_cswin(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size // 2,
                                                                  img_size // 2),
                                                depth=depths[1],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[1])
        self.dowsample_1 = dowsample(embed_dim * 2, embed_dim * 4)
        self.encoderlayer_2 = BasiccswinLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(img_size // (2 ** 2),
                                                                  img_size // (2 ** 2)),
                                                depth=depths[2],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[2])
        self.dowsample_2 = dowsample(embed_dim * 4, embed_dim * 8)
        self.encoderlayer_3 = BasiccswinLayer(dim=embed_dim * 8,
                                                output_dim=embed_dim * 8,
                                                input_resolution=(img_size // (2 ** 3),
                                                                  img_size // (2 ** 3)),
                                                depth=depths[3],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[3])
        self.dowsample_3 = dowsample(embed_dim * 8, embed_dim * 16)

        # Bottleneck
        self.conv = BasiccswinLayer(dim=embed_dim * 16,
                                      output_dim=embed_dim * 16,
                                      input_resolution=(img_size // (2 ** 4),
                                                        img_size // (2 ** 4)),
                                      depth=depths[4],
                                      ...
                                      norm_layer=norm_layer,
                                      use_checkpoint=use_checkpoint,
                                      token_projection=token_projection, token_mlp=token_mlp, se_layer=se_layer,split_size=split_sizes[4])

        # Decoder
        self.upsample_0 = upsample(embed_dim * 16, embed_dim * 8)
        self.decoderlayer_0 = BasiccswinLayer(dim=embed_dim * 16,
                                                output_dim=embed_dim * 16,
                                                input_resolution=(img_size // (2 ** 3),
                                                                  img_size // (2 ** 3)),
                                                depth=depths[5],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[5])
        self.upsample_1 = upsample(embed_dim * 16, embed_dim * 4)
        self.decoderlayer_1 = BasiccswinLayer(dim=embed_dim * 8,
                                                output_dim=embed_dim * 8,
                                                input_resolution=(img_size // (2 ** 2),
                                                                  img_size // (2 ** 2)),
                                                depth=depths[6],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[6])
        self.upsample_2 = upsample(embed_dim * 8, embed_dim * 2)
        self.decoderlayer_2 = BasiccswinLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(img_size // 2,
                                                                  img_size // 2),
                                                depth=depths[7],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[7])
        self.upsample_3 = upsample(embed_dim * 4, embed_dim)
        self.decoderlayer_3 = BasiccswinLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[8],
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,split_size=split_sizes[8])
...
class BasicUniformerLayer_half(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear',token_mlp='ffn',se_layer=False):
...
class Uformer_uniformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
                 embed_dim=32, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
                 win_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        self.encoderlayer_0 = BasicUniformerLayer(dim=embed_dim,
                                                ...
                                                se_layer=se_layer)
        self.dowsample_0 = dowsample(embed_dim, embed_dim * 2)
        self.encoderlayer_1 = BasicUniformerLayer_half(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size // 2,
                                                                  img_size // 2),
                                                depth=depths[1],
    ...
    def flops(self):
        ...
        flops += self.input_proj.flops(self.reso, self.reso)
        # Encoder
        flops += self.encoderlayer_0.flops(self.reso, self.reso) + self.dowsample_0.flops(self.reso, self.reso)
        flops += self.encoderlayer_1.flops(self.reso // 2, self.reso // 2) + self.dowsample_1.flops(self.reso // 2, self.reso // 2)
        flops += self.encoderlayer_2.flops(self.reso // 2 ** 2, self.reso // 2 ** 2) + self.dowsample_2.flops(self.reso // 2 ** 2, self.reso // 2 ** 2)
        flops += self.encoderlayer_3.flops(self.reso // 2 ** 3, self.reso // 2 ** 3) + self.dowsample_3.flops(self.reso // 2 ** 3, self.reso // 2 ** 3)
...
class UNet3(nn.Module):
    def __init__(self, block=ConvBlock3, dim=32):
        ...
        self.ConvBlock1 = block(dim, dim, strides=1)
        self.pool1 = nn.Conv2d(dim, dim*2, kernel_size=4, stride=2, padding=1)
...
if __name__ == "__main__":
    unet=UNet(dim=32)
    uformer=Uformer(img_size=256,embed_dim=16,token_mlp='leff')
    uniformer=Uformer_uniformer(img_size=256,embed_dim=16)
    # print(uformer.flops())
    # print(unet.flops(256,256))
    print(uniformer.flops())
    arch = Uformer_uniformer
    input_size = 256
    # arch = Uformer_Cross
