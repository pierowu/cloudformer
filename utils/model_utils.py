...
def get_arch(opt):
    from model import UNet,Uformer,Uformer_Cross,Uformer_CatCross,Uformer_uniformer,Uformer_cswin,UNet2,UNet3
    arch = opt.arch
    ...
    if arch == 'UNet':
        ...
        model_restoration = UNet2(dim=opt.embed_dim)
    elif arch == 'UNet3':
        model_restoration = UNet3(dim=opt.embed_dim)
    elif arch == 'Uformer':
    ...
    elif arch == 'Uformer_uniformer':
        model_restoration = Uformer_uniformer(img_size=opt.train_ps,embed_dim=opt.embed_dim,win_size=opt.win_size,token_projection=opt.token_projection,token_mlp=opt.token_mlp)
    elif arch == 'Uformer_cswin':
        model_restoration = Uformer_cswin(img_size=opt.train_ps,embed_dim=opt.embed_dim,win_size=opt.win_size,token_projection=opt.token_projection,token_mlp=opt.token_mlp)
    else:
