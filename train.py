...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
logname = os.path.join(log_dir, datetime.datetime.now().__format__("%Y-%d-%m_%H-%M-%S")+'.txt')        #修改时间格式，防止出错
...
if opt.resume:
    ...
    print('------------------------------------------------------------------------------')
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, opt.nepoch, eta_min=1e-6,last_epoch=start_epoch)
...
if opt.warmup:
    ...
    scheduler = StepLR(optimizer, step_size=step, gamma=0.5,last_epoch=start_epoch)
    scheduler.step()
