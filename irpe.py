...
class iRPE(nn.Module):
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        if self.mode == 'bias':
            ...
            test = torch.matmul(
                x,
                self.lookup_table_weight.squeeze(0))
            print('lookuptable bool=', lookup_table.equal(test))
            if RPEIndexFunction is not None:
                return RPEIndexFunction.apply(lookup_table, rp_bucket)
            else:
                a = lookup_table.flatten(2)[:, :, self._ctx_rp_bucket_flatten]. \
                    view(B, -1, L_query, L_key)
                b = lookup_table[:, :, torch.arange(64).unsqueeze(1), rp_bucket]
                print('result bool=', a.equal(b))
...
if __name__ == '__main__':
    ...
    rpe = build_rpe(config, head_dim=2, num_heads=8)
    print(rpe)

    arr = np.random.randn(2, 3,16)
    tensor=torch.Tensor(arr)
    RPEA=RPEAttention(16,rpe_config=config)
    RPEA(tensor)
